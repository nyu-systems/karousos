...
module.exports = class CommentProvider extends Model {
  ...
  static get jsonSchema () {
    return {
      ...
      properties: {
        ...
        ionRequestID:{type:'string'},
        ionTxID:{type:'string'},
        ionTxNum:{type:'integer'},
      }
    }
  }
  ...
  static async initProvider() {
    //console.log(WIKI.models.commentProviders);
   // const commentProvider = await WIKI.models.commentProviders.query().findOne('isEnabled', true)

    /*let commentProvider = (await WIKI.models.knex.transaction(async function(trx){
      //return await WIKI.models.commentProviders.query(trx).findOne('isEnabled', true)
      return trx('commentProviders')
        .select('*')
        .where({
          isEnabled:true
        })
        .from('commentProviders')
