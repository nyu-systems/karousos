...
module.exports = {
   ...
   * Create New Comment
   */
  async create ({ pageId, replyTo, content, user }) {
    // -> Build New Comment
    //console.log(DOMPurify.sanitize(mkdown.render(content)))
    const newComment = {
      content,
      render: "<p>"+content+"<p>",
      replyTo,
...
*/
    // -> Save Comment to DB
    try{
    console.log('tring to create a comment')
    const cm= await WIKI.models.knex.transaction(async(trx)=>{
      return trx('comments').insert(newComment)
    })
    
  //  const cm = await WIKI.models.comments.query().insert(newComment)
  
    console.log("create a new comment successfully")
      }catch(e){
        return -1;
      }
    // -> Return Comment ID
  ...
  async update ({ id, content, user }) {
    const renderedContent = `<p>${content}<p>`
