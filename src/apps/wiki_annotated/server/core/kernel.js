...
module.exports = {
  async init() {
    ...
    //karousos.recordAccess(WIKI.models, requestID, handlerID, false, true,WIKI,"models");


    //onsole.log(WIKI.models)
    try {
  ...
  async preBootMaster() {
    try {
     ...
     //karousos.recordAccess(WIKI.cache, requestID, handlerID, false, true,WIKI,"cache");

      WIKI.scheduler = require('./scheduler').init()
      //karousos.recordAccess(WIKI.scheduler, requestID, handlerID, false, true,WIKI,"scheduler");

      WIKI.servers = require('./servers')
      //karousos.recordAccess(WIKI.servers, requestID, handlerID, false, true,WIKI,"servers");
     ...
     /* WIKI.events = {
      ...
      console.log('finish preBootMaster')
    } catch (err) {
      WIKI.logger.error(err)
      process.exit(1)
    }
