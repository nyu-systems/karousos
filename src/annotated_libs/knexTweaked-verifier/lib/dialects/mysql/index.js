...
Object.assign(Client_MySQL.prototype, {
  ...
  processResponse(obj, runner) {
    ...
    if (obj.output) {
			console.log("PANIC! We do not account for this case");
			process.exit();
      ...
      case 'first': {
		...
		var ret = method === 'first' ? 0 : 1; //junk
		var toSave = {'ret': ret, 'method': method}
		if (karousos.reportCollectionActivated(requestID)){
			var ret2 = karousos.recordStateOp(requestID, handlerID, 'read', ret, this.txId, obj.sql, obj.bindings, runner.builder._single.table);
			requestID = -2;
			handlerID = 'global'; 
			...
			karousos.recordStateOp(requestID, handlerID, 'read', ret, this.txId, obj.sql, this.whereIs, runner.builder._single.table);
			requestID = -2;
			handlerID = 'global'; 
