...
Object.assign(Runner.prototype, {
  ...
  query: async function(obj) {
	...
	//KAROUSOS: turn off the advice collection because the verifier will not do this
	...
	if (karousos.reportCollectionActivated(prevRequestID)){
		var queryPromise = Promise.resolve(obj);
	}else{
	...
	try{
		let resp = await queryPromise;
		requestID = prevRequestID;
	    handlerID = prevHandlerID;
        karousos.setCurrentHandler(requestID, handlerID);
        karousos.pushContext(requestID, handlerID, retEventTypes, objID);
	    let processedResponse = await this.client.processResponse(resp, runner)
	    let fn2 = (processedResponse) => {
