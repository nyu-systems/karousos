...
assign(Builder.prototype, {
  ...
  where(column, operator, value) {
    ...
    if (column === false || column === true) {
      return this.where(1, '=', column ? 1 : 0);
		...
		//turn off the report collect
		karousos.pushContext(requestID, handlerID, retEventTypes, objID);
		requestID = -2;
		handlerID = 'global';
    this.client.whereIs = operator; //KAROUSOS save the primary key
    this.whereIs = operator; //KAROUSOS save the primary key
		[requestID, handlerID, retEventTypes, objID] = karousos.popContext();
  ...
  update(values, returning) {
    ...
    var opInfo = karousos.recordStateOp(requestID, handlerID, 'write', obj, this.client.txId, null, this.whereIs, this._single.table)
    obj.ionRequestID = opInfo[0];
    obj.ionTxID = opInfo[1];
    obj.ionTxNum = opInfo[2];
