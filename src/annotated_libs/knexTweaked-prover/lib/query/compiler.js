    ...
    if (typeof insertData === 'string') {
      ...
      if (insertData.columns.length) {
        //KAROUSOS MODIFICATION
    		insertData.columns = insertData.columns.concat(['ionRequestID', 'ionTxID', 'ionTxNum'])
        delete insertData.columns.objID;
        ...
        while (++i < insertData.values.length) {
          if (i !== 0) sql += '), (';
	   	  	//KAROUSOS MODIFICATION	
		  		var opInfo = karousos.recordStateOp(requestID, handlerID, 'write', [insertData.columns, insertData.values[i]], this.client.txId, sql, insertData.values[i][0], this.tableName)
          delete opInfo.objID
		  		sql += this.formatter.parameterize(
            insertData.values[i].concat([opInfo]),
