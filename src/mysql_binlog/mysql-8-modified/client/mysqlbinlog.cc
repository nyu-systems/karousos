...
static Exit_status process_event(PRINT_EVENT_INFO *print_event_info,
  ...
  */
  if (((rec_count >= offset) &&
	...
	//JENNY:
        bool new_ends_group = ((Query_log_event *)ev)->new_ends_group();
        bool new_starts_group = ((Query_log_event *)ev)->new_starts_group();
	...
	//JENNY:
        //Query_log_event sanity check
        //check trx end statement is commit and not XA end statement or rollbacks
        //check trx start statement is not XA start statement
        if (ends_group) {
          assert((ends_group && new_ends_group) == 1);
        } else if (starts_group) {
          assert((starts_group && new_starts_group) == 1);
        }

        //same logic as in parser.py for switching trx state
        if (new_ends_group) {
          //check if has a trx started
          in_transaction = false;
          print_event_info->skipped_event_in_transaction = false;
          seen_gtid = false;
        } else if (new_starts_group) {
          //check if trx is nested in another trx
