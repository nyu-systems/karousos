...
function csvToTable(file)
	...
	for line in io.lines(file) do
		...
		if firstLine then
			...
			local v1, v2, v3 = line:match("%s*(.*),%s*(.*),%s*(.*)")
			lines[#lines + 1] = {rid = v1, total = v2, k = v3}
		end
	end
	return lines
end
...
function prepareRequests(thread_id)
	...
	for i, data in ipairs(lines) do
		...
		local method = ""
		method = "POST"
		databody = '{"total_accesses":' .. total .. ',"k":' .. k .. '}'
		--- Set up the i-th request of the thread with id thread_id --- 
		requests[thread_id][i] = wrk.format(method, "http://localhost:8000/request", headers, databody)
		request_data[thread_id][i] = {}
		request_data[thread_id][i]["method"] = method
		request_data[thread_id][i]["x-request-id"] = tostring(rid)
		request_data[thread_id][i]["data"] = '{"url": "http://localhost:8000/request", "body":' .. databody ..'}'
