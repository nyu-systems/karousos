...
function csvToTable(file)
	...
	for line in io.lines(file) do
		...
		if firstLine then
			...
			lines[#lines + 1] = {rid = v1, reqtype = v2, date = v3, message = v4, readable = v5} 
		end
	end
	return lines
end
...
function prepareRequests(thread_id)
	...
	for i, data in ipairs(lines) do
		-- The suffix of the date is the random exp number of runVerifier.sh
		local dateSuffix = os.getenv("EXP_NUMBER")
		local rid = data["rid"]
		local reqtype = data["reqtype"]
		local date = data["date"] .. dateSuffix --Randomize date with date_suffix
		local message = data["message"]
		local readable = data["readable"]
		--headers--
		local headers = {}
		...
		local databody = ""
			...
			method = "POST"
			databody = '{"date":"' .. date .. '","message":"' .. message .. '","readable":' .. readable .. '}'
...
response = function(status, headers, body)
	...
	-- Print RETRY if the body of the request contains an ECONNREFUSED error
	if string.find(body, "ECONNREFUSED") then
		print("RETRY")
	end
