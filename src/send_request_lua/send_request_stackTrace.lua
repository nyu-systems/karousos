...
function csvToTable(file)
	...
	for line in io.lines(file) do
		...
		if firstLine then
			...
			local v1, v2, v3, v4 = line:match(postPattern)
			if v1 == nil and v4 == nil then
				v1, v2, v3 = line:match(getPattern)
			end
			if v1 == nil then
				return {{["err"]="Unknown request type"}}
			end
			lines[#lines + 1] = {rid = v1, reqtype = v2, hash = v3, value = v4} 
		end
	end
	return lines
end
...
function prepareRequests(thread_id)
	...
	for i, data in ipairs(lines) do
		...
		local hash = data["hash"] .. os.getenv("EXP_NUMBER") -- randomize hash
		...
		if reqtype == "post" then
			method = "POST"
			databody = '{"hash":"' .. hash .. '","value":"' .. value .. '"}'
		elseif reqtype == "get" then
			reqtype = "check"
			method = "POST"
			databody = '{"hash":' .. '"'.. hash .. '"}'
		elseif reqtype == "list" then
			reqtype = "list"
			method = "POST"
			databody = "{}"
		elseif reqtype == "wait" then
			method = "wait"
			reqtype = ""
			databody = ""
		else
...
request = function()
	...
	if request_idx > reqs_to_send then
		...
		local req_data = request_data[tid][request_idx]
		if req_data["method"] == "wait" then
			--If we are asked to wait then we sleep for some time
			os.execute("sleep " ..  tonumber(3));
			request_idx = request_idx + 1;
			req = requests[tid][request_idx]
			req_data = request_data[tid][request_idx]
		end
		-- HACK HACK HACK
		...
		if id > 1 or ignored_first then
			trace = save_request(trace, os.clock(), init_time, req_data["x-request-id"], req_data["method"], req_data["data"])
		end
